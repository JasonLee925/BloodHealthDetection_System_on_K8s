...
MongoClient.connect(url, function(err, db) {
	...
	app.post('/oxygen', (req, res) => {
		...
		oxygenData['o2'].push(req.body.o2);
		oxygenData['hbeat'].push(req.body.hbeat);
		
		col = db.collection("Oxygen");
		col.insertMany([{
			time:now(), 
			o2:req.body.o2, 
			hbeat:req.body.hbeat}]);
	...
	app.get('/db_oxygen', (req, res) => {
		...
		col.find().forEach(function(doc){
			_o2.push(doc.o2);
			_hbeat.push(doc.hbeat);
		},function(err) {
			console.log({'o2':_o2, 'hbeat':_hbeat});
			res.send({'o2':_o2, 'hbeat':_hbeat});
		});
	});
